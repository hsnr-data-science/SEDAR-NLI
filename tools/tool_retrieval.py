...
class ToolRetriever:
        ...
        try:
            ...
            self.qdrant_client.create_collection(
                ...
                vectors_config=models.VectorParams(
                    size=self.embedding_config.embedding_size,
    ...
    def _invoke_rerank_retriever(
        self, search_kwargs: dict, query: str, k: int = 5
    ) -> list[Document]:
        """
    ...
    def _get_rerank_retriever(
        self, search_kwargs: dict, k: int = 5
    ...
    def _create_document(self, method: Callable, cacheable_class: Any):
        ...
        return Document(
            ...
            metadata={
                ...
                "parent_class": cacheable_class.__name__,
            },
        )

    def _search_by_class(
        self, query: str, cacheable_class_name: str, k: int = 5
    ) -> list[Document]:
        """
    ...
    def get_method_documents(
        self, query: str, k: int = 5, cacheable_class: Any = None
    ) -> list[Document]:
        """
        ...
        Returns:
            list[Document]: A list of documents representing the methods.
        """
        if cacheable_class is not None and CacheableRegistry.is_cacheable(
            cacheable_class
        ):
    ...
    def get_method_descriptions(
        ...
        return "\n\n".join(
            [
                get_minimal_docstring(
                    method_doc.page_content, DOCSTRING_SECTIONS_TO_REMOVE
                )
                for method_doc in documents
            ]
        )
    ...
    def get_class_and_method_descriptions(
        ...
        """
        if cacheable_class:
            return self._describe_cacheable_class(
                query, cacheable_class, k, include_remaining_methods, compress_prompt
            )
        return self._describe_retrieved_classes(
            query, k, include_remaining_methods, describe_all_classes, compress_prompt
        )
    ...
    def _describe_cacheable_class(
        ...
        cacheable_class: Any,
        k: int,
        include_remaining_methods: bool,
        ...
        if include_remaining_methods:
            retrieved_class += "\n\n" + self._get_remaining_methods(
                cacheable_class, retrieved_class, compress_prompt
            )
    ...
    def _describe_retrieved_classes(
        ...
        class_descriptions = "\n\n".join(
            self._describe_single_class(
                class_name, docs, include_remaining_methods, compress_prompt
            )
        ...
        if describe_all_classes:
            return (
                class_descriptions
                + "\n\n"
                + self._describe_unqueried_classes(
                    cacheable_classes, include_remaining_methods, compress_prompt
                )
            )
    ...
    def _describe_single_class(
        ...
        class_description += "\n\n".join(
            self._wrap_method_with_llmlingua(
                get_minimal_docstring(doc.page_content, DOCSTRING_SECTIONS_TO_REMOVE)
            )
            if compress_prompt
            else get_minimal_docstring(doc.page_content, DOCSTRING_SECTIONS_TO_REMOVE)
            for doc in docs
        )
        ...
        if include_remaining_methods:
            ...
            remaining_methods = self._get_remaining_methods(
                cls, retrieved_method_names, compress_prompt
            )
            class_description += "\n\n" + remaining_methods
    ...
    def _describe_unqueried_classes(
        ...
        unqueried_classes = [
            cls for cls in all_classes if cls.__name__ not in queried_classes
        ]

        return "\n\n".join(
            self._describe_single_class(
                cls.__name__, [], include_remaining_methods, compress_prompt
            )
            for cls in unqueried_classes
        )

    def _get_remaining_methods(
        self, cls: Any, retrieved_method_names: list, compress_prompt: bool
    ) -> str:
        ...
        remaining_methods = [
            ...
            if CacheableRegistry.should_use_method(method)
            and method.__name__ not in retrieved_method_names
        ]
    ...
    def get_methods_for_cacheables(
        self, query: str, cacheable_instances: list[Any], k: int = 3
    ) -> str:
        """
        ...
        for cacheable_instance in cacheable_instances:
            ...
            full_cacheables_description += (
                f"{cacheable_class.__name__}\n{cacheable_class.__doc__}\n"
            )
            full_cacheables_description += self.get_method_descriptions(
                query, k, cacheable_class
            )
            full_cacheables_description += "\n"
    ...
    def get_tools(
        self, query: str, tools: list[SedarTool], k: int = 5
    ) -> list[SedarTool]:
        """
        ...
        documents = self.get_method_documents(
            query, k=k, cacheable_class=tools[0].class_instance.__class__
        )
        return [
            tool
            for tool in tools
            if tool.name in [doc.metadata["name"] for doc in documents]
        ]
